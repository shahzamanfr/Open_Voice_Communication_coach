...
import { getTextScenarioFeedback, refineScenarioForStorytelling, getEnhancedStorytellerEvaluation, processUploadedFilesForTeaching } from '../services/geminiService';
...
const StorytellerInterface: React.FC<StorytellerInterfaceProps> = ({ onBack, ai }) => {
  const { theme } = useTheme();
  const [currentStep, setCurrentStep] = useState<'input' | 'refined' | 'storytelling' | 'feedback'>('input');
  ...
  const handleScenarioSubmit = async () => {
    ...
    try {
      ...
      setError("An error occurred while refining your story prompt. Please try again.");
  ...
  const handleStorytellingSubmit = async () => {
    ...
    try {
      ...
      const storytellerFeedback = await getEnhancedStorytellerEvaluation(ai, promptForEval, userStory);
      ...
      <div className="border-b border-gray-200 dark:border-gray-800 p-6">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-semibold text-black dark:text-white">Storyteller Coach</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">Test your storytelling abilities with creative feedback</p>
          </div>
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-center space-x-4">
            <div className={`flex items-center ${currentStep === 'input' ? 'text-white' : currentStep === 'refined' || currentStep === 'storytelling' || currentStep === 'feedback' ? 'text-purple-400' : 'text-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${currentStep === 'input' ? 'bg-white text-black' : currentStep === 'refined' || currentStep === 'storytelling' || currentStep === 'feedback' ? 'bg-purple-500 text-white' : 'bg-gray-700 text-gray-400'}`}>
                1
              </div>
              <span className="ml-2 text-sm">Input Prompt</span>
            </div>
            <div className={`w-16 h-0.5 ${currentStep === 'refined' || currentStep === 'storytelling' || currentStep === 'feedback' ? 'bg-purple-500' : 'bg-gray-700'}`}></div>
            <div className={`flex items-center ${currentStep === 'refined' ? 'text-white' : currentStep === 'storytelling' || currentStep === 'feedback' ? 'text-purple-400' : 'text-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${currentStep === 'refined' ? 'bg-white text-black' : currentStep === 'storytelling' || currentStep === 'feedback' ? 'bg-purple-500 text-white' : 'bg-gray-700 text-gray-400'}`}>
                2
              </div>
              <span className="ml-2 text-sm">Story Prompt</span>
            </div>
            <div className={`w-16 h-0.5 ${currentStep === 'storytelling' || currentStep === 'feedback' ? 'bg-purple-500' : 'bg-gray-700'}`}></div>
            <div className={`flex items-center ${currentStep === 'storytelling' ? 'text-white' : currentStep === 'feedback' ? 'text-purple-400' : 'text-gray-500'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${currentStep === 'storytelling' ? 'bg-white text-black' : currentStep === 'feedback' ? 'bg-purple-500 text-white' : 'bg-gray-700 text-gray-400'}`}>
                3
              </div>
              <span className="ml-2 text-sm">Tell Story</span>
            </div>
            <div className={`w-16 h-0.5 ${currentStep === 'feedback' ? 'bg-purple-500' : 'bg-gray-700'}`}></div>
        ...
        {currentStep === 'input' && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-semibold text-white mb-4">Step 1: Provide Your Story Idea</h2>
              <p className="text-gray-400 max-w-2xl mx-auto">
                Give us any story idea, scenario, or creative prompt you'd like to develop into a story. 
                It can be anything - from a simple character to a complex plot idea.
              </p>
            </div>
            ...
            <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700">
              ...
              <div className="relative">
                <textarea
                  ...
                  className="w-full h-32 p-4 pr-12 bg-gray-800 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-white/50 resize-none"
                  disabled={isLoading}
                />
            ...
            <div className="text-center">
              ...
              >
                {isLoading || fileUploadState.isProcessing 
                  ? (fileUploadState.files.length > 0 ? 'Processing Files...' : 'Crafting Story Prompt...')
                  : (fileUploadState.files.length > 0 ? 'Extract & Create Story Prompt' : 'Create Story Prompt')
                }
              </button>
            </div>
          </div>
        ...
        {currentStep === 'refined' && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-semibold text-white mb-4">Step 2: Your Story Prompt</h2>
              <p className="text-gray-400 max-w-2xl mx-auto">
                Here's your story idea transformed into an engaging, creative prompt. 
                Now you'll tell this story with all your storytelling skills.
              </p>
            </div>

            <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700">
              <h3 className="text-xl font-semibold text-white mb-4">Your Story Prompt:</h3>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 text-center">
              <button
                onClick={() => { setUseOwnNarration(false); setCurrentStep('storytelling'); }}
                className="px-6 py-3 bg-white text-black rounded-lg font-medium hover:bg-gray-100 transition-colors duration-200"
              >
                Use This Prompt
              </button>
              <button
                onClick={() => { setUseOwnNarration(true); setCurrentStep('storytelling'); }}
        ...
        {currentStep === 'storytelling' && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-semibold text-white mb-4">Step 3: Tell Your Story</h2>
              <p className="text-gray-400 max-w-2xl mx-auto">
                Now tell the story based on the prompt above. Be creative, engaging, and captivating. 
                Show your storytelling skills with vivid descriptions, compelling characters, and an engaging narrative.
              </p>
            </div>

            {!useOwnNarration && (
              <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700 mb-6">
                <h3 className="text-lg font-semibold text-white mb-3">Story Prompt:</h3>
                <div className="bg-gray-900/50 rounded-lg p-4">
                  <p className="text-gray-300 leading-relaxed whitespace-pre-wrap">{refinedScenario}</p>
                </div>
              </div>
            ...
            <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700">
              ...
              <textarea
                ...
                className="w-full h-64 p-4 bg-gray-800 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-white/50 resize-none"
                disabled={isLoading}
              />
            </div>
        ...
        {currentStep === 'feedback' && feedback && (
          <div className="space-y-6">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-semibold text-white mb-4">Your Storytelling Performance</h2>
              <p className="text-gray-400 max-w-2xl mx-auto">
                Here's how well you told your story. The Storyteller AI has evaluated your creativity, 
                narrative flow, and engagement skills.
              </p>
            </div>
            ...
            <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700 hover:border-gray-600 transition-all duration-300">
              <div className="flex items-center justify-center mb-6">
                <h3 className="text-xl font-semibold text-white">Overall Storytelling Score</h3>
              </div>
            ...
            <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700 hover:border-gray-600 transition-all duration-300 hover:shadow-lg hover:shadow-white/5">
              <h3 className="text-xl font-semibold text-white mb-4">Storyteller's Assessment</h3>
              <p className="text-gray-300 leading-relaxed mb-4">{feedback.feedback}</p>
              
              <div className="space-y-4">
                <div>
                  <h4 className="font-medium text-white mb-2">Storytelling Strengths:</h4>
            ...
            {/* Improvement Tips */}
            <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700 hover:border-gray-600 transition-all duration-300 hover:shadow-lg hover:shadow-white/5">
              <h3 className="text-xl font-semibold text-white mb-4">Personalized Storytelling Recommendations</h3>
            ...
            {feedback.storytellingAnalysis && (
              <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700 hover:border-gray-600 transition-all duration-300 hover:shadow-lg hover:shadow-white/5">
                <h3 className="text-xl font-semibold text-white mb-4">Detailed Storytelling Analysis</h3>
                <div className="space-y-4">
                  <div>
                    <h4 className="font-medium text-white mb-2">Your Strongest Storytelling Moment:</h4>
                    <p className="text-gray-300 italic">"{feedback.storytellingAnalysis.strongestMoment}"</p>
                  </div>
                  <div>
                    <h4 className="font-medium text-white mb-2">Your Weakest Storytelling Moment:</h4>
                    <p className="text-gray-300 italic">"{feedback.storytellingAnalysis.weakestMoment}"</p>
                  </div>
                  <div>
                    <h4 className="font-medium text-white mb-2">Best Storytelling Technique:</h4>
                    <p className="text-gray-300 italic">"{feedback.storytellingAnalysis.bestTechnique}"</p>
                  </div>
                  <div>
                    <h4 className="font-medium text-white mb-2">Missed Storytelling Opportunities:</h4>
                    <p className="text-gray-300">{feedback.storytellingAnalysis.missedOpportunities}</p>
                  </div>
                  <div>
                    <h4 className="font-medium text-white mb-2">Emotional Connection:</h4>
                    <p className="text-gray-300">{feedback.storytellingAnalysis.emotionalConnection}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {feedback.communicationBehavior && (
              <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700 hover:border-gray-600 transition-all duration-300 hover:shadow-lg hover:shadow-white/5">
                <h3 className="text-xl font-semibold text-white mb-4">Your Storytelling Style</h3>
                <div className="space-y-4">
                  <div>
                    <h4 className="font-medium text-white mb-2">Storytelling Profile:</h4>
            ...
            {feedback.exampleRewrite && (
              <div className="bg-gray-800/30 rounded-lg p-6 border border-gray-700 hover:border-gray-600 transition-all duration-300 hover:shadow-lg hover:shadow-white/5">
                <h3 className="text-xl font-semibold text-white mb-4">Example Storytelling Improvement</h3>
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    <h4 className="font-medium text-white mb-2">Improved Storytelling:</h4>
