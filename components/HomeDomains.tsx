      ...
      {domains.map((domain) => {
        ...
        return (
          <button
            ...
            className={`group relative flex h-full min-h-[140px] flex-col justify-between overflow-hidden rounded-2xl border border-gray-800 bg-gradient-to-br from-gray-950 via-black to-gray-950 p-5 text-left shadow-[0_10px_40px_-20px_rgba(0,0,0,0.7)] transition-transform duration-300 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-gray-600 ${isSelected ? 'ring-2 ring-white/30' : ''}`}
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-xs text-gray-400 leading-relaxed line-clamp-2">{domain.description}</p>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-3 gap-2">
              {domain.images.slice(0, 3).map((img) => (
                <div key={img.id} className="relative aspect-[4/3] overflow-hidden rounded-lg bg-gray-900">
